...
def generate_charge_summary(all_profiles_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        t_RMS_col = all_profiles_df.columns[7] # 假设T_min在第6列
        p_max_col = all_profiles_df.columns[8] # 假设p_max在最后一列
    except IndexError:
        print("[✘] 错误: 数据文件中的列数不足。请确保文件包含所有必需的列。")
        return pd.DataFrame() # 返回空DataFrame

    # 根据“制冷剂”和“单管流量”进行分组
    for (coolant_index, qmm, qmj), group_df in all_profiles_df.groupby(by=[coolant_col, qmm_col, qmj_col]):
