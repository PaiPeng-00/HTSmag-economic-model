...
def main() -> None:
    ...
    audit = {
        ...
        "timestamp_utc": datetime.now(timezone.utc).isoformat(), "device": "arc_16pancake_nuc600",
        "scenario": "S2", "coolant": "He", "git_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(),
        "source": {"v6c_boundaries": {"path": str(BOUNDARIES.relative_to(ROOT)), "sha256": sha256(BOUNDARIES)},
                   "v6c_brackets": {"path": str(BRACKETS.relative_to(ROOT)), "sha256": sha256(BRACKETS)}},
        ...
        "outputs": {path.name: {"path": str(path.relative_to(ROOT)), "rows": len(pd.read_csv(path)), "sha256": sha256(path)} for path in (OUT_COVERAGE, OUT_SENSITIVITY)},
