#!/usr/bin/env python3
"""Build V6.2 frozen-B1 strict-feasible Parquet partitions and a compact audit.

Only the strict denominator is persisted.  The full anchored CSV is deliberately
not rewritten: rejected rows remain recoverable from the compact B grid and are
represented by auditable status/gate counts.
"""
...
def main() -> None:
    ...
    strict_root = out / "v6_2_b1_strict_feasible_parquet"
    audit_path = out / "v6_2_b1_strict_parquet_audit.json"
    if strict_root.exists() or audit_path.exists():
        raise FileExistsError(f"refusing existing V6.2-B1 Parquet output in: {out}")
    ...
    strict_parts: list[dict[str, object]] = []
    rows = strict_rows = 0
    ...
    for raw in raw_paths:
        for chunk in pd.read_csv(raw, chunksize=CHUNK, low_memory=False):
            ...
            strict_frame = frame.loc[strict].copy()
            if not strict_frame.empty:
                # Partition keys are part of the schema and permit predicate reads without a 6+ GB CSV pass.
                table = pa.Table.from_pandas(strict_frame, preserve_index=False)
                pq.write_to_dataset(table, root_path=str(strict_root), partition_cols=["scenario", "Top_K", "coolant"], compression="zstd", existing_data_behavior="overwrite_or_ignore")
    ...
    part_files = sorted(strict_root.rglob("*.parquet"))
    if not part_files:
        raise AssertionError("strict feasibility produced no Parquet partitions")
    part_manifest = []
    for part in part_files:
        meta = pq.ParquetFile(part).metadata
        part_manifest.append({"path": str(part.relative_to(out)), "rows": int(meta.num_rows), "bytes": int(part.stat().st_size), "sha256": sha256(part)})
    if sum(item["rows"] for item in part_manifest) != strict_rows:
        raise AssertionError("Parquet partition row count does not equal strict denominator")
    audit = {"experiment_id":"V6.2-B1-strict-feasible-parquet","status":"PASS_SMOKE" if args.max_rows is not None else "PASS","device":DEVICE,"rows":rows,"strict_feasible_rows":strict_rows,"status_counts":status_counts,"matrix_sha256":next(iter(matrices)),"capex_reconstruction_max_abs_residual_USD":capex_max_residual,"frozen_transform_script_sha256":anchor_sha,"capital_boundary_version":anchor.CAPITAL_BOUNDARY_VERSION,"storage":{"format":"partitioned_parquet","compression":"zstd","partition_columns":["scenario","Top_K","coolant"],"strict_root":strict_root.name,"parts":part_manifest},"source":{"raw_stage":str(raw_stage),"availability":str(args.availability.resolve()),"availability_sha256":sha256(args.availability.resolve())},"started_utc":started,"completed_utc":datetime.now(timezone.utc).isoformat()}
