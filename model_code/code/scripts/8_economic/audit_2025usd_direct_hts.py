"""Audit the direct-2025-US$ HTS scenario rerun against plant-v3."""
...
OLD_BOUNDARY = "plant_v3_core_pcs_coolant_2025usd"
...
def main() -> None:
    ...
    invariance = compare_nonmonetary_exact(args.baseline, args.new)
    old = normalize(args.baseline, OLD_BOUNDARY)
    new = normalize(args.new, COST_BOUNDARY_VERSION)
    if len(old) != len(new):
        raise AssertionError(f"Row count changed: baseline={len(old)}, direct={len(new)}")
    ...
    metrics_path = args.output_dir / "economic_2025usd_direct_hts_change_metrics.csv"
    metrics.to_csv(metrics_path, index=False)

    conversion_path = args.output_dir / "economic_2025usd_direct_hts_conversion_audit.csv"
    ...
    frontier_path = args.output_dir / "robust_frontier_5pct_2025usd_direct_hts.csv"
    ...
    if not (
        ...
        raise AssertionError("Jointly feasible counts changed after the direct HTS scenario-price revision")
    ...
    if len(robust_5) != 1:
        raise AssertionError("Direct-HTS 5% robust count/range was not uniquely reproduced")

    invariance_path = args.output_dir / "economic_2025usd_direct_hts_invariance_audit.json"
    invariance_payload = {
        "price_basis_year": PRICE_BASIS_YEAR,
        "old_scan": str(args.baseline),
        "old_sha256": sha256(args.baseline),
        "new_scan": str(args.new),
    ...
    summary_path = args.output_dir / "economic_2025usd_direct_hts_change_summary.md"
    report = [
        "# Direct constant-2025-US$ HTS scenario rerun audit",
        "",
        f"- New cost boundary: `{COST_BOUNDARY_VERSION}`",
        f"- Plant-v3 baseline scan SHA256: `{sha256(args.baseline)}`",
        f"- Direct-HTS 2025-USD scan SHA256: `{sha256(args.new)}`",
        f"- Rows: {len(new):,}",
        ...
        *[f"- {key}: `{value:.10g}`" for key, value in accounting.items()],
        "",
        "The exact conversion ledger is in `economic_2025usd_direct_hts_conversion_audit.csv`; the full machine-readable metrics are in `economic_2025usd_direct_hts_change_metrics.csv`.",
    ]
