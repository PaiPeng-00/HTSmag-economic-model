...
def define_parameters():
    ...
    # 比例系数 β = 0.34 / V_magnet(SPARC) ; V_magnet(SPARC) 与 cfg.V_magnet 同一公式(device.py),
    # 几何定义的系统因子在比值中抵消 -> 等价于按真实磁体体积比缩放. 回路体积随本器件几何驱动.
    TFMC_HE_INTERNAL_M3 = 0.34       # TFMC 绕组包+plena 内部 SHe 体积 (Michael 2024)
    TFMC_V_MAGNET_M3    = 0.9053     # SPARC 几何(NP16/L1=3.0/R1=0.4/R2=0.3/WID4/DIST10)同公式 V_magnet
    EXT_PIPING_FACTOR   = 1.5        # 外部管线裕量 (论文实测 0.46/0.34=1.35)
    cryo_loop_volume_m3 = (EXT_PIPING_FACTOR
                           * (TFMC_HE_INTERNAL_M3 / TFMC_V_MAGNET_M3)
                           * cfg.V_magnet * cfg.Ntf)
