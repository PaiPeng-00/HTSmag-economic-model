...
# 新增器件只需加一个 yaml, 无需改代码。派生量(V_WP、V_mag 等)仍由本文件自动计算。
# ------------------------------------------------------------------------------
import os as _os
try:
...
# TF磁体个数
Ntf = _dev("Ntf", 18)
S_outer = (R1 + R2)**2 * np.pi / 2 + (R1 + R2 + L1 / 2)**2 * np.pi / 2
S_inner = R1**2 * np.pi / 2 + (R1 + L1 / 2)**2 * np.pi / 2
S_WP = S_outer - S_inner + R2 * L1
H_WP = (DIST + WID) * NP
V_WP = S_WP * H_WP  # 含饼间距的绕组包包络体积

# 线圈盒及其他冷结构只增加核热积分体积；不改变匝长、电感、电阻、峰值场或冷却剂缩放。
STRUCTURAL_VOLUME_FRACTION = _dev("STRUCTURAL_VOLUME_FRACTION", 0.50)
V_struct = STRUCTURAL_VOLUME_FRACTION * V_WP
V_mag = V_WP + V_struct

# 向后兼容只读别名；新代码须显式选择 V_WP（冷却剂）或 V_mag（核热）。
S_magnet = S_WP
H_magnet = H_WP
V_magnet = V_mag
...
NUCLEAR_POWER_DENSITY = _dev("NUCLEAR_POWER_DENSITY", 421.0)          # 体积平均核功率密度 (W/m^3)
VOLUME_M3 = V_mag                      # 绕组包加50%冷结构的核热积分体积 (m^3)
