...
def _scan_grid(config_path: Path) -> tuple[list[float], list[int], list[float]]:
    ...
    npw_raw = raw["npw_values"]
    if isinstance(npw_raw, dict) and "range" in npw_raw:
        spec = npw_raw["range"]
        npw_values = list(range(
            int(spec["start"]), int(spec["stop"]) + 1, int(spec.get("step", 1))
        ))
    else:
        npw_values = [int(value) for value in npw_raw]
